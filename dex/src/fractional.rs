...
pub fn int_sqrt(m: i128) -> Result<i128, DexErrorCode> {
    let mut start = 0_i128;
    let mut sq_root = 0_i128;
    if m < 0 {
        Err(DexErrorCode::SqrtRootError)
    } else if m == 0 {
        Ok(0)
    } else if m > 1 {
        let mut end = 2;
...
pub fn int_div(m: u128, other: u128) -> Result<u128, DexErrorCode> {
    if other == 0 {
        Err(DexErrorCode::DivisionbyZero)
    } else {
        Ok(m / other)
    }
}
pub fn u64_to_quote(a: u64) -> Result<Fractional, DexErrorCode> {
    if a > (i64::MAX) as u64 {
        Err(DexErrorCode::NumericalOverflow)
    } else {
        Ok(Fractional {
            m: (a as i64),
            exp: 0,
...
impl Fractional {
    ...
    pub fn from_str(s: &str) -> Result<Fractional, DexError> {
        match s.split_once(".") {
            Some((lhs, rhs)) => {
                let m = format!("{}{}", lhs, rhs)
                    .parse::<i64>()
                    .map_err(|_| DexErrorCode::DeserializeError)?;
    ...
    pub fn reduce_from_i128(m: &mut i128, exp: &mut u64) -> Result<Self, DexErrorCode> {
        if *m == 0 {
            *exp = 0;
        }
        ...
        if !num_in_i64(*m) || *exp > EXP_UPPER_LIMIT {
            return Err(DexErrorCode::NumericalOverflow);
        }

        Ok(Fractional::new(*m as i64, *exp))
    }

    pub fn reduce_from_i128_unchecked(mut m: i128, mut exp: u64) -> Result<Self, DexErrorCode> {
        if m == 0 {
            exp = 0;
        }
    ...
    pub fn reduce(m: &mut i128, exp: &mut u64, precision: u64) -> Result<Self, DexError> {
        if *m == 0 {
            return Ok(Fractional::new(0, 0));
        }
        while *exp > precision {
            if *m % 10 != 0 {
                return Err(DexErrorCode::RoundError.into());
            }
            *m /= 10;
            *exp -= 1;
        }
        if !num_in_i64(*m) {
            return Err(DexErrorCode::NumericalOverflow.into());
        }
        Ok(Fractional::new(*m as i64, *exp))
    }

    pub fn round_unchecked(&self, digits: u32) -> Result<Fractional, DexError> {
        let diff = digits as i32 - self.exp as i32;
        if diff >= 0 {
            Ok(Fractional::new(
                (self.m)
                    .checked_mul(POW10[diff as usize])
                    .ok_or(DexErrorCode::NumericalOverflow)?,
                digits as u64,
            ))
        } else {
            Ok(Fractional::new(
    ...
    pub fn round(&self, digits: u32) -> DexResult<Fractional> {
    ...
    fn round_up(&self, digits: u32) -> Result<i64, DexErrorCode> {
    ...
    pub fn checked_add(&self, other: impl Into<Fractional>) -> Result<Fractional, DexErrorCode> {
    ...
    pub fn checked_sub(&self, other: impl Into<Fractional>) -> Result<Fractional, DexErrorCode> {
        let other = other.into();
        let (mut m, mut exp) = if self.exp > other.exp {
            (
                self.m as i128 - other.round_up(self.exp as u32)? as i128,
    ...
    pub fn checked_mul(&self, other: impl Into<Fractional>) -> Result<Fractional, DexError> {
        let other = other.into();
    ...
    pub fn checked_div(&self, other: impl Into<Fractional>) -> Result<Fractional, DexErrorCode> {
        let other = other.into();
        let sign = self.sign() * other.sign();
    ...
    pub fn sqrt(&self) -> Result<Fractional, DexErrorCode> {
    ...
    pub fn exp(&self) -> Result<Fractional, DexErrorCode> {
        let x = *self;
...
impl FromStr for Fractional {
    ...
    #[inline]
    fn from_str(s: &str) -> Result<Fractional, DexError> {
        match s.split_once(".") {
            Some((lhs, rhs)) => {
                let m = format!("{}{}", lhs, rhs)
                    .parse::<i64>()
